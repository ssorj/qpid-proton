...
static void pni_unmap_remote_channel(pn_session_t *ssn)
{
  ...
  if (pn_hash_get(transport->remote_channels, channel)) {
    pni_endpoint_decref(&ssn->endpoint);
  }
...
static inline void endpoint_set_remote_state(pni_endpoint_t *endpoint, uint8_t remote_state)
{
  endpoint->state = (endpoint->state & PN_LOCAL_MASK) | remote_state;
}

static void pni_post_remote_open_events(pn_transport_t *transport, pn_connection_t *connection) {
    pni_collector_put_object(connection->collector, connection, PN_CONNECTION_REMOTE_OPEN);

    if (transport->remote_idle_timeout) {
      pni_collector_put_object(connection->collector, transport, PN_TRANSPORT);
    }
}
...
int pn_transport_bind(pn_transport_t *transport, pn_connection_t *connection)
{
  ...
  if (transport->open_rcvd) {
    endpoint_set_remote_state(&connection->endpoint, PN_REMOTE_ACTIVE);
...
int pn_transport_unbind(pn_transport_t *transport)
{
  ...
  pni_collector_put_object(conn->collector, conn, PN_CONNECTION_UNBOUND);
  ...
  pni_endpoint_t *endpoint = conn->endpoint_head;
  while (endpoint) {
...
static void pni_map_remote_handle(pn_link_t *link, uint32_t handle)
{
  ...
  pni_endpoint_incref(&link->endpoint);
}
...
static void pni_maybe_post_closed(pn_transport_t *transport)
{
  ...
  if (transport->head_closed && transport->tail_closed) {
    pni_collector_put_object(collector, transport, PN_TRANSPORT_CLOSED);
  }
}
...
static void pni_close_tail(pn_transport_t *transport)
{
  if (!transport->tail_closed) {
    ...
    pn_collector_t *collector = pni_transport_collector(transport);
    pni_collector_put_object(collector, transport, PN_TRANSPORT_TAIL_CLOSED);
...
int pn_do_error(pn_transport_t *transport, const char *condition, PN_PRINTF_FORMAT const char *fmt, ...)
{
  ...
  pn_collector_t *collector = pni_transport_collector(transport);
  pni_collector_put_object(collector, transport, PN_TRANSPORT_ERROR);
...
int pn_do_open(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  pn_connection_t *conn = transport->connection;
  if (conn) {
    endpoint_set_remote_state(&conn->endpoint, PN_REMOTE_ACTIVE);
...
int pn_do_begin(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  endpoint_set_remote_state(&ssn->endpoint, PN_REMOTE_ACTIVE);
  pni_collector_put_object(transport->connection->collector, ssn, PN_SESSION_REMOTE_OPEN);
  return 0;
}

static pn_link_t *pni_find_link(pn_session_t *ssn, pn_bytes_t name, bool is_sender)
{
  pni_endpoint_type_t type = is_sender ? SENDER : RECEIVER;
...
int pn_do_attach(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  endpoint_set_remote_state(&link->endpoint, PN_REMOTE_ACTIVE);
  ...
  pni_collector_put_object(transport->connection->collector, link, PN_LINK_REMOTE_OPEN);
  return 0;
}
...
int pn_do_transfer(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  if (delivery) {
    ...
    pni_collector_put_object(transport->connection->collector, delivery, PN_DELIVERY);
  }
...
int pn_do_flow(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  pni_collector_put_object(transport->connection->collector, ssn, PN_SESSION_FLOW);
  ...
  if (handle_init) {
    ...
    pni_collector_put_object(transport->connection->collector, link, PN_LINK_FLOW);
  }
...
int pn_do_detach(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  if (closed)
  {
    endpoint_set_remote_state(&link->endpoint, PN_REMOTE_CLOSED);
    pni_collector_put_object(transport->connection->collector, link, PN_LINK_REMOTE_CLOSE);
  } else {
    pni_collector_put_object(transport->connection->collector, link, PN_LINK_REMOTE_DETACH);
  }
...
int pn_do_end(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  pn_condition_set(condition, cond, desc, info);
  endpoint_set_remote_state(&ssn->endpoint, PN_REMOTE_CLOSED);
  pni_collector_put_object(transport->connection->collector, ssn, PN_SESSION_REMOTE_CLOSE);
...
int pn_do_close(pn_transport_t *transport, uint8_t frame_type, uint16_t channel, pn_bytes_t payload)
{
  ...
  endpoint_set_remote_state(&conn->endpoint, PN_REMOTE_CLOSED);
  pni_collector_put_object(transport->connection->collector, conn, PN_CONNECTION_REMOTE_CLOSE);
  return 0;
}
...
static int pni_process_conn_setup(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static inline int pni_map_local_channel(pn_session_t *ssn)
{
  ...
  pni_endpoint_incref(&ssn->endpoint);
  return 1;
}

static int pni_process_ssn_setup(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static int pni_process_link_setup(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static int pni_process_flow_receiver(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
  ...
  *settle = false;
  ...
  if ((int16_t) ssn_state->local_channel >= 0 && (int32_t) link_state->local_handle >= 0) {
    if (!state->sent && (delivery->done || pn_buffer_size(delivery->bytes) > 0) &&
      ...
      pni_collector_put_object(transport->connection->collector, link, PN_LINK_WORK);
    }
  }
...
static int pni_process_tpwork(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static int pni_process_flush_disp(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static int pni_process_flow_sender(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static int pni_process_link_teardown(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
...
static int pni_process_ssn_teardown(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
  if (endpoint->type == SESSION)
  {
    pn_session_t *session = (pn_session_t *) endpoint;
...
static int pni_process_conn_teardown(pn_transport_t *transport, pni_endpoint_t *endpoint)
{
  if (endpoint->type == CONNECTION)
  {
...
static inline int pni_phase(pn_transport_t *transport, int (*phase)(pn_transport_t *, pni_endpoint_t *))
{
  ...
  while (endpoint)
  {
    pni_endpoint_t *next = endpoint->transport_next;
...
static void pni_close_head(pn_transport_t *transport)
{
  if (!transport->head_closed) {
    ...
    pn_collector_t *collector = pni_transport_collector(transport);
    pni_collector_put_object(collector, transport, PN_TRANSPORT_HEAD_CLOSED);
    pni_maybe_post_closed(transport);
  }
}
