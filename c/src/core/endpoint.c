...
static pn_connection_t *endpoint_connection(pni_endpoint_t *endpoint)
{
...
static pn_event_type_t endpoint_event(pni_endpoint_type_t type, bool open) {
  switch (type) {
  case CONNECTION:
...
static pn_event_type_t endpoint_final_type(pni_endpoint_type_t type) {
  switch (type) {
  case CONNECTION:
...
static inline void endpoint_set_local_state(pni_endpoint_t *endpoint, uint8_t local_state)
{
  endpoint->state = (endpoint->state & PN_REMOTE_MASK) | local_state;
}
...
{
  endpoint->type = (pni_endpoint_type_t) type;
...
void pni_endpoint_free_children(pn_list_t *children, pn_list_t *freed)
{
  while (pn_list_size(children) > 0) {
    pni_endpoint_t *endpoint = (pni_endpoint_t *) pn_list_get(children, 0);
    assert(!endpoint->referenced);
    pn_free(endpoint);
  }

  while (pn_list_size(freed) > 0) {
    pni_endpoint_t *endpoint = (pni_endpoint_t *) pn_list_get(freed, 0);
    assert(!endpoint->referenced);
    pn_free(endpoint);
  }
...
static bool endpoint_matches(pni_endpoint_t *endpoint, pni_endpoint_type_t type, pn_state_t state)
{
...
{
  while (endpoint) {
    if (endpoint_matches(endpoint, type, state)) return endpoint;
...
static pni_endpoint_t *endpoint_parent(pni_endpoint_t *endpoint) {
  switch (endpoint->type) {
  case CONNECTION:
    return NULL;
  case SESSION:
...
static bool endpoint_live(pni_endpoint_t *endpoint) {
  switch (endpoint->type) {
  case CONNECTION:
...
static bool endpoint_bound(pni_endpoint_t *endpoint)
{
  pn_connection_t *conn = endpoint_connection(endpoint);
  pn_session_t *ssn;
  pn_link_t *lnk;
